...
const buildUser = (id: number, username: string): Item.UserInfo => ({
  id,
  username,
  nickname: username,
  avatar: '',
...
describe('reaction utils', () => {
  ...
  it('sorts larger reaction groups first and summarizes overflow', () => {
    ...
    const summary = summarizeReactionGroups(
      reactions.sort((left, right) => right.count - left.count),
      2,
    );
