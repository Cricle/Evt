...
import { LEGACY_DEFAULT_SPACE_SLUG, normalizeDefaultSpaceSlug, resolveSpaceSlug, } from '@/utils/spaces';
export const useStoreProfile = defineStore("profile", () => {
    ...
    const currentSpaceSlug = computed({
        ...
        set: (value) => {
            currentSpaceSlugState.value = resolveSpaceSlug(value, profile.value.defaultSpaceSlug);
        },
    });
    const spaces = ref([]);
    const activeSpaceMembers = ref([]);
    function loadDefaultSiteProfile() {
        ...
        profile.value.defaultSpaceSlug = normalizeDefaultSpaceSlug(import.meta.env.VITE_DEFAULT_SPACE_SLUG);
        ...
        profile.value.defaultTweetMaxLength = Number(import.meta.env.VITE_DEFAULT_TWEET_MAX_LENGTH);
        profile.value.tweetWebEllipsisSize = Number(import.meta.env.VITE_TWEET_WEB_ELLIPSIS_SIZE);
        profile.value.tweetMobileEllipsisSize = Number(import.meta.env.VITE_TWEET_MOBILE_ELLIPSIS_SIZE);
        profile.value.defaultTweetVisibility =
            import.meta.env.VITE_DEFAULT_TWEET_VISIBILITY.toLowerCase();
        profile.value.defaultMsgLoopInterval = Number(import.meta.env.VITE_DEFAULT_MSG_LOOP_INTERVAL);
    ...
    function updateSiteProfile(data) {
        profile.value.enableSpaces = data.enable_spaces ?? profile.value.enableSpaces;
        profile.value.defaultSpaceSlug = normalizeDefaultSpaceSlug(data.default_space_slug ?? profile.value.defaultSpaceSlug);
        if (!currentSpaceSlug.value ||
