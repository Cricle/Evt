...
import type { AppThemePreset } from '@/store/main';
...
export const themePresetDefinitions: Record<AppThemePreset, ThemePresetDefinition> = {
  emerald: {
...
const getPalette = (preset: AppThemePreset, isDark: boolean) =>
  isDark ? themePresetDefinitions[preset].dark : themePresetDefinitions[preset].light;
...
export const buildThemeCssVars = (preset: AppThemePreset, isDark: boolean) => {
  const palette = getPalette(preset, isDark);
