...
import { EvtImageUploadAdapter, normalizeUploadedAssetUrl, normalizeLinkInput, resolveUploadAuthHeader, uploadFilesSequentially, } from '@/components/ckeditor-upload-plugin';
describe('ckeditor upload plugin', () => {
    ...
    it('normalizes relative uploaded asset urls to api urls', () => {
        ...
        expect(normalizeUploadedAssetUrl('v1/media/42')).toBe('/v1/media/42');
        expect(normalizeUploadedAssetUrl('https://cdn.evt.example/v1/media/42')).toBe('https://cdn.evt.example/v1/media/42');
    });
    ...
    it('uploads non-image assets sequentially and keeps lifecycle balanced on partial failure', async () => {
        ...
        await uploadFilesSequentially(files, 'attachment', { onStart, onUploaded, onFinish, onError }, upload);
    ...
    it('uploads editor images and returns the ckeditor default payload', async () => {
        ...
        const adapter = new EvtImageUploadAdapter({ file: Promise.resolve(file) }, { onStart, onUploaded, onFinish, onError }, vi.fn().mockResolvedValue({
            kind: 'public/image',
    ...
    it('propagates image upload failures after notifying plugin hooks', async () => {
        const onError = vi.fn();
        const adapter = new EvtImageUploadAdapter({ file: Promise.resolve(new File(['x'], 'broken.png', { type: 'image/png' })) }, { onError }, vi.fn().mockRejectedValue(new Error('upload failed')));
        await expect(adapter.upload()).rejects.toThrow('upload failed');
        expect(onError).toHaveBeenCalledTimes(1);
    });
});
//# sourceMappingURL=ckeditor_upload_plugin.test.js.map
